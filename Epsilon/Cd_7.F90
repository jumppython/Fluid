...
!  
  ...
  INTEGER :: LOOP=0
!
  INTEGER :: N1999=14999999
  INTEGER :: N2000=15000000
  INTEGER :: N2480=24999999
  INTEGER :: N2500=25000000
  INTEGER :: NN0=1
!  
  REAL :: RTIME=0.0
  REAL :: PRMT=0.0
...
!
  ...
  OPEN (UNIT=710,FILE='AVGU.DAT')
  OPEN (UNIT=720,FILE='AVGV.DAT')
  OPEN (UNIT=730,FILE='AVGP.DAT')
!
  TPMAX = -1000.0
  TPMIN = 1000.0
!
...
!	DO J=2,MY-1
!	  DO K=2,MX-1
!	    U(I,J)=U(I,J)+(RDM-.5)*.000001
!		V(I,J)=V(I,J)+(RDM-.5)*.000001
!	  END DO
!	END DO
!
!*************************************************************
...
!*************************************************************
!                                                   
    IF(NN .GE. N2000 .AND. MOD(NN, MWRITE) .EQ. 0) THEN
	  NWRITED=NWRITED+1
...
!
      DO J=1,15
	    AVG1U=AVG1U+U(A1XY(J,1),A1XY(J,2))
		AVG1V=AVG1V+V(A1XY(J,1),A1XY(J,2))
		AVG1P=AVG1P+PDIFF(A1XY(J,1),A1XY(J,2))
!
        AVG2U=AVG2U+U(A2XY(J,1),A2XY(J,2))
		AVG2V=AVG2V+V(A2XY(J,1),A2XY(J,2))
		AVG2P=AVG2P+PDIFF(A2XY(J,1),A2XY(J,2))
!
		AVG3U=AVG3U+U(A3XY(J,1),A3XY(J,2))
		AVG3V=AVG3V+V(A3XY(J,1),A3XY(J,2))
		AVG3P=AVG3P+PDIFF(A3XY(J,1),A3XY(J,2))
!
		AVG4U=AVG4U+U(A4XY(J,1),A4XY(J,2))
		AVG4V=AVG4V+V(A4XY(J,1),A4XY(J,2))
		AVG4P=AVG4P+PDIFF(A4XY(J,1),A4XY(J,2))
	  END DO
!
      AVGU=AVG1U+AVG2U+AVG3U+AVG4U
	  AVGV=AVG1V+AVG2V+AVG3V+AVG4V
	  AVGP=AVG1P+AVG2P+AVG3P+AVG4P
!
      AVG1U=AVG1U/15.0
	  AVG1V=AVG1V/15.0
	  AVG1P=AVG1P/15.0
...
!
	  ...
	  END DO
!
      WRITE(710,"(F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8)") TIME,AVG1U,AVG2U,AVG3U,AVG4U,AVGU
	  WRITE(720,"(F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8)") TIME,AVG1V,AVG2V,AVG3V,AVG4V,AVGV
	  WRITE(730,"(F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8,' ',F18.8)") TIME,AVG1P,AVG2P,AVG3P,AVG4P,AVGP
!
	END IF
!
!*************************************************************
...
!*************************************************************
!
    IF(NN .GE. N2480 .AND. MOD(NN,VWRITE) .EQ. 0) THEN
	  WRITE(INDX, "(I7)") NN
	  INDX=ADJUSTL(INDX)
!
...
!
! READ THE DATA FROM P549.TXT AND UV549.TXT
!
  IMPLICIT NONE
  INTEGER, INTENT (IN) :: N
...
!
  ...
  JMAX = 252
!
  OPEN (UNIT=80,FILE="UVP1999.DAT",STATUS="OLD")
!
  DO I = 1, IMAX
    JSTEP = 1
    DO J = 1, JMAX
